...
class ChatUpdateRequest(BaseModel):
    """Request to update chat inclusion status and/or display name."""

    included: Optional[bool] = Field(
        None, description="Whether to include (true) or exclude (false) the chat"
    )
    chat_name: Optional[str] = Field(
        None, description="New display name for the chat (e.g. to distinguish an imported chat from its live counterpart)"
    )


class ChatBulkActionRequest(BaseModel):
    """Request to apply an action to multiple chats at once."""

    chat_ids: list[str] = Field(..., description="Chat IDs to act on")
    action: str = Field(
        ..., description="One of: 'include', 'exclude', 'delete', 'exclude_and_delete'"
    )
