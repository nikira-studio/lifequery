...
from telethon import TelegramClient, errors, utils as telethon_utils
...
async def _extract_forward_info(message, client: TelegramClient) -> dict[str, Any]:
    """Return provenance for a Telegram-forwarded message.

    Telegram identifies the person who forwarded a message as ``message.sender``.
    The original author lives in ``message.fwd_from`` and may be partially hidden
    by Telegram's privacy settings.  Keep every available identifier rather than
    replacing the author with the forwarding sender.
    """
    ...
    # Telegram uses saved_from_* when a message was forwarded out of Saved
    # Messages, so fall back to those fields rather than losing provenance.
    source_peer = getattr(forward, "from_id", None) or getattr(forward, "saved_from_id", None)
    source_id: str | None = None
    source_name = (
        getattr(forward, "from_name", None)
        or getattr(forward, "saved_from_name", None)
        or getattr(forward, "post_author", None)
        or None
    )
    if source_peer is not None:
        try:
            source_id = str(telethon_utils.get_peer_id(source_peer))
        except Exception:
            source_id = str(getattr(source_peer, "user_id", "") or getattr(source_peer, "channel_id", "") or "") or None

        # A direct forward normally has an ID but no display name. Resolve it
        # while the Telegram client is connected; a failure is expected for
        # privacy-protected/deleted accounts and must not discard the forward.
        if not source_name:
            try:
                source_name = _derive_sender_name(await client.get_entity(source_peer))
            except Exception:
                pass

    source_chat_id = getattr(forward, "channel_id", None)
    if source_chat_id is None:
        source_chat_id = getattr(forward, "saved_from_peer", None)
        if source_chat_id is not None:
            try:
                source_chat_id = telethon_utils.get_peer_id(source_chat_id)
            except Exception:
                source_chat_id = None
    ...
    return {
        "is_forwarded": True,
        "forward_sender_id": source_id,
        "forward_sender_name": source_name,
        "forward_date": int(forward_date.timestamp()) if forward_date else None,
        "forward_chat_id": str(source_chat_id) if source_chat_id is not None else None,
        "forward_message_id": str(getattr(forward, "channel_post", None) or getattr(forward, "saved_from_msg_id", None) or "") or None,
    }
    ...
    async with _telethon_lock:
        ...
        try:
            ...
            for dialog in dialogs:
                # Use Telethon's marked peer id (matches what the live listener's
                # message.chat_id already returns) so the same physical chat never
                # fragments into two chat_id variants between sync and live updates.
                        ...
                        async with _write_lock:
                            ...
                            try:
                                ...
                                for message in batch:
                                    ...
                                    forward_info = await _extract_forward_info(message, client)
...
async def ensure_chat_entry(
    ...
    message_count/last_message_at are only written when explicitly provided —
    callers that merely touch metadata (e.g. the live listener on every new
    message) must not overwrite the running count with a stale/default value.
    """
    ...
    if existing:
        if message_count is not None and last_message_at is not None:
            await execute_write(
                """
                UPDATE chats SET
                    chat_name = ?,
        ...
        await execute_write(
            ...
            VALUES (?, ?, ?, 1, ?, ?, ?)
            """,
            (chat_id, chat_name, chat_type, message_count or 0, last_message_at or 0, created_at),
        )
...
async def _insert_message(
    ...
    forward_sender_id: str | None = None,
