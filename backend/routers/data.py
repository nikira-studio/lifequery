...
async def update_chat(chat_id: str, request: ChatUpdateRequest = Body(...)) -> dict:
    """Update chat inclusion status and/or display name.

    Renaming is mainly useful for imported chats (e.g. "Contact (archived)" to
    distinguish from the live-synced "Contact") — a live-synced chat's name
    will be overwritten again by the next Telegram sync/listener event.
    """
    try:
        ...
        if request.included is None and request.chat_name is None:
            raise HTTPException(status_code=400, detail="No fields to update")

        if request.included is not None:
            await execute_write(
                "UPDATE chats SET included = ? WHERE chat_id = ?",
                (1 if request.included else 0, chat_id),
            )
        if request.chat_name is not None:
            new_name = request.chat_name.strip()
            if not new_name:
        ...
        logger.info(
            f"Updated chat {chat_id}: included={request.included}, chat_name={request.chat_name}"
        )
        return {"ok": True}
...
async def _fetch_live_dialog_ids() -> Optional[set[str]]:
    """Fetch the current Telegram dialog list as a set of marked chat_ids.

    Returns None if not connected or the fetch fails for any reason — callers
    should treat that as "can't verify, don't assume gone" rather than
    "confirmed absent."
    """
    from telegram.telethon_sync import _load_session_string, get_telegram_status
    ...
    try:
        ...
        try:
            ...
            return {str(telethon_utils.get_peer_id(d.entity)) for d in dialogs}
        finally:
            await client.disconnect()
    except Exception as e:
        logger.warning(f"Couldn't fetch live Telegram dialogs: {e}")
        return None


async def _delete_chat_internal(
    chat_id: str, live_dialog_ids: Optional[set[str]] = None
) -> dict:
    """Delete a single chat's messages/chunks from LifeQuery.

    Shared by the single-chat and bulk delete endpoints. If live_dialog_ids is
    provided and this chat isn't in it, the chat row is fully purged instead
    of kept around as an excluded/empty ghost — matching what "Purge Ghosts"
    already does, just at delete time instead of as a separate step.
    """
    import asyncio
    ...
    # 3. OR we know the live dialog list and this chat isn't in it (no longer
    #    a contact/member) — imports are exempt since they're intentionally
    #    kept separate from live Telegram state.
    is_manual_only = all(s == 'json_import' for s in sources)
    has_no_messages = len(sources) == 0
    no_longer_a_contact = (
        live_dialog_ids is not None
        and chat_id not in live_dialog_ids
        and not chat_id.startswith("import_")
    )

    should_delete_record = is_manual_only or has_no_messages or no_longer_a_contact
...
async def delete_chat(chat_id: str) -> dict:
    """Delete chat messages and chunks from LifeQuery.

    Fully removes the chat row if it's confirmed no longer a live Telegram
    contact/member (checked here, not as a separate purge step); otherwise
    marks it excluded so it doesn't get re-synced.
    """
    logger.info(f"Delete chat requested: {chat_id}")
    try:
        live_dialog_ids = await _fetch_live_dialog_ids()
        return await _delete_chat_internal(chat_id, live_dialog_ids)
    except HTTPException:
        raise
    except Exception as e:
...
@router.post("/chats/bulk")
async def bulk_chat_action(request: ChatBulkActionRequest = Body(...)) -> dict:
    """Apply include/exclude/delete to multiple chats in one call.

    Lets the UI offer a checkbox multi-select instead of forcing a
    toggle-then-reload cycle per chat.
    """
    action = request.action
    chat_ids = request.chat_ids

    if action not in ("include", "exclude", "delete", "exclude_and_delete"):
        raise HTTPException(status_code=400, detail=f"Unknown action: {action}")
    if not chat_ids:
        return {"ok": True, "processed": 0, "messages_deleted": 0, "chunks_deleted": 0}
    ...
    if action in ("include", "exclude"):
        ...
        await execute_write(
            f"UPDATE chats SET included = ? WHERE chat_id IN ({placeholders})",
            (1 if action == "include" else 0, *chat_ids),
        )
        processed = len(chat_ids)
    else:
        # delete / exclude_and_delete: fetch the live dialog list once for the
        # whole batch (not per chat_id — one Telegram round trip either way),
        # then run per-chat so one bad chat_id doesn't abort the whole batch.
        live_dialog_ids = await _fetch_live_dialog_ids()
        for chat_id in chat_ids:
            try:
                result = await _delete_chat_internal(chat_id, live_dialog_ids)
                messages_deleted += result["messages_deleted"]
                chunks_deleted += result["chunks_deleted"]
                processed += 1
            except Exception as e:
                logger.warning(f"bulk_chat_action: failed to delete {chat_id}: {e}")
                errors.append({"chat_id": chat_id, "error": str(e)})

    logger.info(
        f"Bulk chat action '{action}' on {len(chat_ids)} chats: "
...
@router.post("/chats/purge-ghosts")
async def purge_ghost_chats() -> dict:
    """Permanently remove empty chat rows that are no longer live contacts.

    A "ghost" is a chat with zero real messages left in LifeQuery. When
    connected to Telegram, we additionally require it to be absent from the
    current dialog list — e.g. someone you deleted and blocked, so their
    empty placeholder row can be fully cleared out. If they reappear later,
    they'll just show up again as a new chat, same as any first sync.

    This used to run opportunistically on every GET /api/chats whenever the
    Telegram connection status looked disconnected — but that status check
    races against the live listener's own connection and can misfire,
    silently deleting rows at unpredictable moments. It's now an explicit,
    on-demand action instead. Delete (single and bulk) now does this same
    check inline, so this endpoint mainly exists to sweep up chats that were
    deleted while Telegram was unreachable and fell back to soft-exclude.
    """
    live_dialog_ids = await _fetch_live_dialog_ids()
    ...
    async with _write_lock:
        ...
        try:
            ...
            if live_dialog_ids is not None:
                to_remove = [
                    cid for cid in candidates
                    if cid not in live_dialog_ids and not cid.startswith("import_")
                ]
            else:
            ...
            for cid in to_remove:
                cur = await db.execute("DELETE FROM chats WHERE chat_id = ?", (cid,))
    ...
    logger.info(
        f"Purged {removed} ghost chat(s)"
        + (" (cross-checked against live dialogs)" if live_dialog_ids is not None else " (offline fallback)")
    )
    return {"ok": True, "removed": removed, "checked_live": live_dialog_ids is not None}
...
async def sync_chats_generator() -> AsyncGenerator[ServerSentEvent, None]:
    ...
    try:
        ...
        telegram_chat_ids = {str(telethon_utils.get_peer_id(d.entity)) for d in dialogs}
        updated_count = 0
        new_count = 0
        removed_count = 0
        ...
        async with _write_lock:
            ...
            try:
                ...
                for dialog in dialogs:
                    chat_id = str(telethon_utils.get_peer_id(dialog.entity))
...
async def get_stats() -> StatsResponse:
    ...
    try:
        ...
        # Embedded count — read the real ChromaDB collection rather than the
        # SQLite embedded_at flag, which only records that an embed batch was
        # *attempted* and can go stale if the vector store write it was
        # tracking later gets rolled back or deleted (e.g. a failed reindex
        # swap) without the flag being cleared.
        from vector_store.chroma import _get_collection

        try:
            embedded_count = await asyncio.to_thread(_get_collection().count)
        except Exception as e:
            logger.warning(f"Could not get ChromaDB count for stats: {e}")
            embedded_count = 0
...
async def reindex_generator() -> AsyncGenerator[ServerSentEvent, None]:
    """SSE generator for reindex operation, guarded by the shared sync lock.

    Reindex used to run without acquiring _sync_lock at all, so a "Sync Now"
    or "Process Now" click during a reindex would run fully concurrently —
    both hammering Ollama and racing on the same SQLite chunks rows.
    """
    if _sync_lock.locked():
        yield create_error_event("A sync or process operation is already in progress.")
        return
    async with _sync_lock:
        async for event in _reindex_generator_inner():
            yield event


async def _reindex_generator_inner() -> AsyncGenerator[ServerSentEvent, None]:
    start_ts = int(datetime.now().timestamp())
    total_chunks = 0
    try:
        ...
        try:
            ...
            # chunk_messages_streaming() only processes messages newer than each
            # chat's last_chunked_at watermark, so that must be reset too —
            # otherwise every already-chunked chat looks "up to date" and a
            # reindex silently produces almost no chunks.
            yield create_progress_event("reindex", "Clearing old chunks...")
            await execute_write("DELETE FROM chunks")
            await execute_write("UPDATE chats SET last_chunked_at = NULL")
