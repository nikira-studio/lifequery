...
def _message_from_row(row: dict) -> AgentMessageRecord:
    return AgentMessageRecord(
        ...
        forward_sender_id=row["forward_sender_id"],
        forward_sender_name=row["forward_sender_name"],
...
async def _query_messages(
    ...
    rows = await fetch_all(
        ...
        SELECT
            ...
            c.chat_type, m.sender_id, m.sender_name,
            m.is_forwarded, m.forward_sender_id, m.forward_sender_name,
            m.forward_date, m.forward_chat_id, m.forward_message_id, m.text,
