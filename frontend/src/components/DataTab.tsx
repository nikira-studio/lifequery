...
export function DataTab() {
  ...
  const bulkDeleteConfirmRef = useRef<HTMLDialogElement>(null);
  const [selectedChatIds, setSelectedChatIds] = useState<Set<string>>(
    new Set(),
  );
  const [bulkActionPending, setBulkActionPending] =
    useState<BulkChatAction | null>(null);
  ...
  const handlePurgeGhosts = async () => {
    if (purgingGhosts) return;
    setPurgingGhosts(true);
    try {
      const result = await purgeGhostChats();
      setSyncChatsState({
        status: "success",
        message: result.checked_live
          ? `Removed ${result.removed} chat(s) with no messages that are no longer live contacts`
          : `Removed ${result.removed} empty chat(s) (offline — didn't cross-check against live contacts)`,
        lastRun: new Date().toLocaleTimeString(),
      });
      await refreshChats();
    } catch (err) {
  ...
  const requestBulkAction = (action: BulkChatAction) => {
    if (selectedChatIds.size === 0 || bulkActionPending) return;

    if (action === "delete" || action === "exclude_and_delete") {
      // Route destructive actions through the confirmation dialog first.
      setBulkActionPending(action);
      bulkDeleteConfirmRef.current?.showModal();
      return;
    }
  ...
  const executeBulkAction = async (action: BulkChatAction) => {
    const chatIds = Array.from(selectedChatIds);
    setSyncChatsState({
      status: "running",
      message: `Applying "${action.replace(/_/g, " ")}" to ${chatIds.length} chat(s)...`,
    });

    try {
      const result = await bulkChatAction(chatIds, action);
      const actionLabel =
        action === "include"
          ? "Included"
          : action === "exclude"
      ...
      setSyncChatsState({
        ...
        message:
          result.errors.length > 0
            ? `${actionLabel} ${result.processed} chat(s), ${result.errors.length} failed`
            : `${actionLabel} ${result.processed} chat(s)${
                result.messages_deleted
                  ? ` (${result.messages_deleted} messages, ${result.chunks_deleted} chunks removed)`
                  : ""
              }`,
        lastRun: new Date().toLocaleTimeString(),
      });
  ...
  const confirmBulkDelete = async () => {
    ...
    bulkDeleteConfirmRef.current?.close();
    if (!action) return;
        ...
        <div className="bg-card border border-border rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex-shrink-0 flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-secondary text-muted-foreground hover:text-foreground hover:bg-secondary/70 transition-all disabled:opacity-40"
                title="Permanently remove empty chats that are no longer live contacts"
              >
                {purgingGhosts ? (
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-medium bg-primary/10 text-primary hover:bg-primary/20 transition-all disabled:opacity-40"
                title={
                  !isTelegramConnected
          ...
          <div className="flex flex-col gap-3">
            ...
            {selectedChatIds.size > 0 && (
              <div className="flex items-center justify-between gap-2 px-3 py-2 bg-primary/5 border border-primary/20 rounded-lg animate-in fade-in slide-in-from-top-2 duration-200">
                <span className="text-xs font-medium text-foreground">
                  {selectedChatIds.size} selected
                </span>
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => requestBulkAction("include")}
                    disabled={!!bulkActionPending}
                    className="px-2.5 py-1 rounded-md text-xs font-medium bg-secondary hover:bg-secondary/70 transition-colors disabled:opacity-40"
                  >
                    Include
                  </button>
                  <button
                    onClick={() => requestBulkAction("exclude")}
                    disabled={!!bulkActionPending}
                    className="px-2.5 py-1 rounded-md text-xs font-medium bg-secondary hover:bg-secondary/70 transition-colors disabled:opacity-40"
                  >
                    Exclude
                  ...
                  <button
                    ...
                    disabled={!!bulkActionPending}
                    className="px-2.5 py-1 rounded-md text-xs font-medium bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors disabled:opacity-40"
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => requestBulkAction("exclude_and_delete")}
                    disabled={!!bulkActionPending}
                    className="px-2.5 py-1 rounded-md text-xs font-medium bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors disabled:opacity-40"
                    title="Exclude from memory search and remove existing messages"
                  >
                    Exclude &amp; Delete
                  </button>
                  <button
            ...
            <div className="border border-border rounded-lg overflow-hidden">
              {/* Header Row */}
              <div className="grid grid-cols-[24px_1fr_80px_80px] gap-2 px-3 py-2 bg-muted/50 border-b border-border items-center">
                <input
                  type="checkbox"
                  className="w-3.5 h-3.5"
                  checked={
                    filteredChats.length > 0 &&
                    filteredChats.every((c) => selectedChatIds.has(c.chat_id))
                  }
                  onChange={() =>
                    toggleSelectAllVisible(filteredChats.map((c) => c.chat_id))
                  }
                  title="Select all visible chats"
                />
              ...
              <div className="max-h-[400px] overflow-y-auto divide-y divide-border">
                {chatsLoading ? (
                  ...
                  filteredChats.map((chat) => (
                    <div
                      key={chat.chat_id}
                      className="grid grid-cols-[24px_1fr_80px_80px] gap-2 px-3 py-2 hover:bg-secondary/50 transition-colors items-center"
                    >
                      <input
                        type="checkbox"
                        className="w-3.5 h-3.5"
                          ...
                          {renamingChatId === chat.chat_id ? (
                            <div className="flex items-center gap-1.5">
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") confirmRenameChat(chat.chat_id);
                                  if (e.key === "Escape") cancelRenameChat();
                                }}
                                className="flex-1 min-w-0 px-2 py-1 text-sm bg-background border border-primary rounded text-foreground focus:outline-none"
                              />
                              <button
                                onClick={() => confirmRenameChat(chat.chat_id)}
                                className="flex-shrink-0 p-1 text-success hover:bg-secondary rounded"
                                title="Save name"
                              >
                              ...
                              <button
                                ...
                                className="flex-shrink-0 p-1 text-muted-foreground hover:bg-secondary rounded"
                                title="Cancel"
                              >
                            ...
                            <div className="flex items-center gap-2 group/name">
                              ...
                              {chat.chat_id.startsWith("import_") && (
                                <span
                                  className="flex-shrink-0 px-1.5 py-0.5 rounded text-xs bg-primary/10 text-primary"
                                  title="From a JSON import — kept separate from any live Telegram chat, safe to rename permanently"
                                >
                                  Imported
                                </span>
                              )}
                              ...
                              <button
                                ...
                                className="flex-shrink-0 p-0.5 text-muted-foreground hover:text-foreground opacity-0 group-hover/name:opacity-100 transition-opacity"
                                title={
                                  chat.chat_id.startsWith("import_")
                                    ? "Rename chat"
                                    : "Rename chat (will be overwritten by the next Telegram sync)"
                                }
                              >
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-foreground">
                {bulkActionPending === "exclude_and_delete"
                  ? "Exclude & Delete Chats"
                  : "Delete Chats"}
              </h3>
              <p className="text-sm text-muted-foreground mt-2">
                Are you sure you want to delete messages for{" "}
                <span className="font-medium text-foreground">
                  {selectedChatIds.size} chat(s)
                </span>
                ? This removes them from LifeQuery only (not Telegram) and
                cannot be undone.
              </p>
            </div>
          </div>
